...
router.post("/predict-priority", auth_1.requireAuth, auth_1.requireAdmin, async (req, res) => {
    try {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (text.includes("HIGH"))
                        return res.json({ priority: Priority.HIGH });
                    if (text.includes("LOW"))
                        return res.json({ priority: Priority.LOW });
...
function keywordPriorityAnalysis(description) {
